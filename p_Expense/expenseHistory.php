...
//query to test if we can pull out records from the order tables.
            $makerName = "";
            $result = mysql_query("SELECT DISTINCT `orderNo` FROM `order` WHERE `date` BETWEEN '$from' AND '$to' $normalSearch ORDER BY `date` DESC");
    		while ($row = mysql_fetch_assoc($result)){
    		    $orderMakerSetting = $row['orderNo'];
    		    for($i=1;$i<21;$i++){
    		...
    		$result = mysql_query("SELECT * FROM `order`  WHERE `date` BETWEEN '$from' AND '$to' $normalSearch GROUP BY `tformNo` ORDER BY `date` DESC");
    		while ($row = mysql_fetch_assoc($result)){
    		    $tformNo = $row['tformNo'];
