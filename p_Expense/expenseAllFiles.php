												...
												while ($row = mysql_fetch_assoc($result)){
												   $id = $row['id'];
												    ...
												    for($i = 1; $i < 20; $i++){
														...
														$tempDate[] = array($row['orderNo_'.$i], $row['date_'.$i]);
												        }
												        $orderArray = array_unique($temp);
												        foreach ($orderArray as $value => $key){
															if ($key != ''){
																echo $key." "; // echo orderNo
																
															// print the date with the orderNo
															/* ------- */
															
															foreach ($tempDate as $value2 => $key2){
																if ($key2[0] == $key && $key2[1] != "" && $key2[1] != "0000-00-00"){
																	echo "<span style='font-size: 9px;'>(".$key2[1].")</span>";
																} else {
																	echo "";
																}
															}
